...
def iniciar_cuestionario(cuestionario_id):
    ...
    db.session.flush()
    temp=requests.get(url='http://cuestionarios:8080/cuestionario/'+cuestionario_id, json={'usuariocuestionario': usuariocuestionario_nuevo.id})
    db.session.commit()
...
def responder_opcion():
    ...
    peticion_pregunta=requests.get(url='http://cuestionarios:8080/pregunta', json={'texto':texto_pregunta,'id_cuestionario':usuariocuestionario.id_cuestionario})
    id_pregunta=peticion_pregunta.json()['id_pregunta']
    ##Le pido al servicio que controla los cuestionarios la opcion
    peticion_opcion=requests.get(url='http://cuestionarios:8080/opcion', json={'texto':texto_opcion,'id_pregunta':id_pregunta})
    ...
    nueva_respuesta=RespuestaUsuario(id_pregunta=id_pregunta,id_opcion=id_opcion,texto=texto_opcion, id_cuestionario_usuario=id_usuariocuestionario, valor=valor)
    ...
    peticion_numero_preguntas=requests.get(url='http://cuestionarios:8080/numero_preguntas/'+str(usuariocuestionario.id_cuestionario))
...
def obtener_resultados():
    ...
    y=requests.get(url='http://cuestionarios:8080/cuestionario/resultados', json=lista)
    lista=[]
    for t in y.json():
        tempo_cuest=CuestionarioUsuario.query.filter(CuestionarioUsuario.id==t['usuariocuestionario']).first()
        t['puntuacion']=tempo_cuest.puntuacion
        for p in t['preguntas']:
            peticion_pregunta=requests.get(url='http://cuestionarios:8080/pregunta', json={'texto':p['pregunta'],'id_cuestionario':tempo_cuest.id_cuestionario})
            id_pregunta=peticion_pregunta.json()['id_pregunta']
            temp_resp=RespuestaUsuario.query.filter(RespuestaUsuario.id_pregunta==id_pregunta).filter(RespuestaUsuario.id_cuestionario_usuario==tempo_cuest.id).first()
            p['respuesta']=temp_resp.texto
            if temp_resp.valor>0:
                p['resultado']='correcta'
            else:
                p['resultado']='incorrecta'
        lista.append(t)
    return jsonify(lista)

@app.route('/valor_respuesta', methods=['GET'])
def obtener_valor():
    return jsonify({"hola": "no quiero entrar"})
    data= request.get_json()
    return jsonify({"id_cuestionario_usuario": data["id_usuario_cuestionario"], "id_pregunta": data["id_pregunta"]})
    respuesta_usuario=RespuestaUsuario.query.filter(id_cuestionario_usuario==data['id_usuario_cuestionario']).filter(RespuestaUsuario.id_pregunta==data['id_pregunta']).first()

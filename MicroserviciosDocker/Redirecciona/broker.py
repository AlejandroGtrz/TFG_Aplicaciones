...
def crear_usuario():
    ...
    url='http://usuarios:5050/register'
    x = requests.post(url, json= data)
    return jsonify(x.json())
@app.route('/login')
def login():
    data = request.get_json()
    url="http://usuarios:5050/login"
    x = requests.get(url, json=data)
    return jsonify(x.json()) 
@app.route('/cuestionario', methods=['POST'])
def crear_cuestionario():
    data = request.get_json()
    url="http://cuestionarios:8080/cuestionario"
...
def iniciar_cuestionario(cuestionario_id):
    url="http://operaciones:8000/resolver/"+cuestionario_id
    headers = {'x-access-token': request.headers['x-access-token']}
...
def obtener_resultados():
    ...
    url="http://operaciones:8000/cuestionario/resultados"
...
def borrar_cuestionario(id_cuestionario):
    url="http://cuestionarios:8080/cuestionario/"+id_cuestionario
    headers = {'x-access-token': request.headers['x-access-token']}
    x = requests.delete(url, headers= headers)
    return jsonify(x.json())
@app.route('/opcion', methods=['POST'])
def responder_opcion():
    url="http://operaciones:8000/opcion"
    data=request.get_json()
    headers = {'x-access-token': request.headers['x-access-token']}
    x = requests.post(url,json=data, headers= headers)
    return jsonify(x.json())

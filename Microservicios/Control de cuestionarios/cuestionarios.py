...
def borrar_cuestionario( id_cuestionario):
    ...
    if(id_usuario==cuestionario.id_creador):
        ...
        requests.delete(url='http://0.0.0.0:8000/cuestionario_usuario', json={'id_cuestionario': id_cuestionario})
    else:
...
def obtener_resultados():
    ...
    for d in data:
        ...
        y={'titulo': cuestionario.titulo, 'preguntas': "" , 'puntuacion': ''}
        ...
        for p in preguntas:
            ...
            x=requests.get(url='http://0.0.0.0:8000/valor', json={'id_pregunta': p.id, 'id_usuario_cuestionario': d['id']})
            valor= x.json()['valor']
            correcta='incorrecta'
            if valor>0:
                correcta='correcta'
            ...
            temp=Opciones.query.filter(Opciones.id==x.json()['opcion']).first()
            l.append({'pregunta': p.texto, 'opciones': l2, 'respuesta': temp.texto_opcion, 'resultado': correcta})
        y['preguntas']=l
        response=requests.get(url='http://0.0.0.0:8000/puntuacion/'+str(d['id']))
        y['puntuacion']=response.json()['puntuacion']
        lista.append(y)
